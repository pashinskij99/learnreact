          ...
          <div className="row">
            <div className="col-12 col-lg-12">
              <ul className="list-group">
                {errors[0] && errors.map(error => <li key={error.message} className="list-group-item text-danger">{error.message}</li>)}
              </ul>
                  ...
                  <div className="form-group col-12 col-md-6">
                    <select name="category" onChange={handleInputChange} id className="form-control form-control-lg">
                      ...
                      {categories[0] && categories.map(category =>
                        <option key={category.id} value={category.id}>{category.name.substring(0, 10)}</option>)}
                    </select>
                  </div>
                </div>
